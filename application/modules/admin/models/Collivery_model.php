...
class Collivery_Model extends CI_Model
{
    ...
    public function order_place_update($order_id, $collivery_id)
    {
        $this->db->where('id', $order_id);
        return $this->db->update('orders', array('order_placed' => 1, "collivery_id" => $collivery_id));
    }
    ...
    {
        $this->db->select('o.id, o.user_id, o.price, o.ship_address_id, o.ship_contact_id, od.product_sku, od.product_name, od.quantity, od.weight, od.length, od.width, od.hight, od.option1, od.option2, od.option3');
    ...
    {
        $query = $this->db->select('id, user_id, collivery_id')->from("orders")->where(array('order_placed' => 1, 'delivered' => 0, 'collivery_id !=' => 0))->get();
        if($query->num_rows() > 0)
        {
            $result = $query->result();
            return $result;
