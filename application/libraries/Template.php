...
class Template
{
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		{        
            ...
            $layout           = $this->_layout ? $this->_layout : 'layout';

            
			// Find the main body and 3rd param means parse if its a theme view (only if parser is enabled)
			$this->enable_parser(false);
			$this->_body =  self::_load_view($layout, $this->_data, TRUE, self::_find_view_folder());
		}
    ...
    /**
     * 
     * @param string $image image name for the product
     * @param string $option thumb for thumbnaiL and empty for fuLL size
     * @return string IMAGE path 
     */
    public function product_image($image, $option = "thumb"){
        
        $image_path = site_url("assets/img/".$image);
        ...
        {
            return site_url("assets/img/not_found.png"); 
        }
    }
    
    /**
     * @param int $count TotaL data to dispLay.
     * @param int $perpage TotaL count in a page
     * @return string pagination Link to the List
     */
    public function paginate($count, $perpage = 20)
    {
        $CI =& get_instance();
        $CI->load->library('pagination');

        $config['base_url']   = site_url("products/paginate");
